...
parser.add_argument("--loss_virtual_weight", type=float, default=0.001, help="压制训练时候,loss_virtual的权重")
# v5版本专用 选择ae参数
...
# 起势
print(str(args))
name_runs = f"runs/yazhixunlian_genev3--ae_version{args.ae_version}--loss_weight{args.loss_virtual_weight}"
writer = SummaryWriter(name_runs)
writer.add_text("args", str(args))
name_args = get_args_str(args)
name_project = "train_resnet18_bygenev3"
...
for epoch in range(args.epochs):
    ...
    for batch_idx, (data, label) in enumerate(trainloader_cifar10):
        ...
        data_virtual, label_virtual = model_g.generate_virtual_v3(data, label, set_encoded_detach=True,

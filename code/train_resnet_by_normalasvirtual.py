...
for epoch in range(args.epochs):
    ...
    for batch_idx, (data, label) in enumerate(trainloader_cifar10):
        ...
        data_virtual, label_virtual = data.detach(), (torch.ones([len(label), num_classes]) / num_classes).cuda()
...
with open(f"./runs/{name_project}.txt", "a+") as results_file:
    ...
    results_file.write(
        f"--lossweight{args.loss_virtual_weight}--ae_versionnormal : acc={acc},mmc_cifar10={mmc_cifar10},mmc_cifar100={mmc_cifar100},mmc_svhn={mmc_svhn}\r\n")

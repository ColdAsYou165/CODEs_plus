...
class BasicBlock(nn.Module):
    def __init__(self, inplanes, planes, stride=1, adv=False):
        ...
        if inplanes != planes:
            self.flag = False
            self.downsample = nn.ModuleList([nn.Conv2d(inplanes, planes, kernel_size=1, stride=stride, bias=False),
                                             Mixturenorm2d(planes, adv)])
        else:
            self.flag = True
...
class ResNet(nn.Module):
    def __init__(self, block, n_size, num_classes):
        print("num_classes:",num_classes)
        super(ResNet, self).__init__()
        self.inplane = 16
        self.conv1 = nn.Conv2d(3, self.inplane, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = Mixturenorm2d(self.inplane)
        self.relu = nn.ReLU(inplace=True)
    ...
    def _make_layer(self, block, planes, blocks, stride):
        ...
        for stride in strides:
            layers.append(block(self.inplane, planes, stride, adv=False))
            self.inplane = planes

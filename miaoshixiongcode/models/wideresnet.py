...
class BasicBlock(nn.Module):
    def __init__(self, in_planes, out_planes, stride, dropRate=0.0):
        ...
        self.bn2 = Mixturenorm2d(out_planes)
    ...
    def forward(self, x,adv=False):
        if not self.equalInOut:
            x = self.relu1(self.bn1(x,adv))
        else:
            out = self.relu1(self.bn1(x,adv))
        out = self.relu2(self.bn2(self.conv1(out if self.equalInOut else x),adv))
        if self.droprate > 0:
...
class NetworkBlock(nn.Module):
    ...
    def _make_layer(self, block, in_planes, out_planes, nb_layers, stride, dropRate):
        ...
        for i in range(int(nb_layers)):
            layers.append(block(i == 0 and in_planes or out_planes, out_planes, i == 0 and stride or 1, dropRate)) # block的初始化
...
class WideResNet(nn.Module):
    def __init__(self, depth, num_classes, widen_factor=1, dropRate=0.0):
        ...
        self.bn1 = Mixturenorm2d(nChannels[3])
    ...
    def forward(self, x,adv=False):
        out = self.conv1(x)
        out = self.block1(out,adv)
        out = self.block2(out,adv)
